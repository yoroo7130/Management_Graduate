...
namespace A_오징어_EduMgr
{
    class ProDBManager
    {
        ...
        // DB 연결
        public SqlConnection ProDBConnector()
        {
            // 윈도우 인증 계정일 경우엔
        ...
        // SELECT
        public DataSet RowQuery(string procedure, params string[] values)
        {
            try
            {
                using (conn = ProDBConnector())
                {
        ...
        public int ExecQuery(string procedure, params string[] values)
        {
            try
            {
                using (conn = ProDBConnector())
